...
if(isset($_POST['submit'])){
	...
	foreach($questionid as $value){
		...
		if($strarraycount==1){
			...
			$ansid = 0;
			if($langid != 1){
			    $translation = $translate->translate($strarray[0], [
                    'target' => 'en'
                ]);

                $ansttxt = $translation['text'];
			}else{
			    $ansttxt = $strarray[0]; 
			}
	...
	if(isset($_POST['school_name']) && !empty($_POST['school_name'])){
		...
		$insert_to_school_name =  dbRowInsert("answers",$data_school_name);

		if($insert_to_school_name){
	...
	reDirect("survey-thankyou.php?msg=".$msg."&surveyid=".$_REQUEST['surveyid']."&langid=".$langid);
...
$languages = explode(',',$row_get_survey['language']);
...
foreach($languages as $key=>$val){
	...
	if($row_get_language['id'] == $langid){
		...
		$langText['your_first_name'] = $row_get_language['your_first_name_text'];

		$langText['your_last_name'] = $row_get_language['your_last_name_text']; 

		$langText['your_email'] = $row_get_language['your_email_text'];

		$langText['your_phone_number'] = $row_get_language['your_phone_number_text'];

		$langText['continue_next_step'] = $row_get_language['continue_next_step_text'];

		$langText['back'] = $row_get_language['back_text'];

		$langText['finish'] = $row_get_language['finish_text'];

		$langText['continue'] = $row_get_language['continue_text'];

		$langText['yes'] = $row_get_language['yes_text'];

		$langText['no'] = $row_get_language['no_text'];

		$langText['required_error'] = $row_get_language['required_error_text'];
...
if($row_get_survey['isStep'] == 1){
	...
	while($row_get_surveys_steps = mysqli_fetch_assoc($get_surveys_steps)){
		...
		$survey_steps[$row_get_surveys_steps['id']]['title'] = ($lang_iso == 'en')?$row_get_surveys_steps['step_title']:$row_get_surveys_steps['step_title_'.$lang_iso];
...
while($row_get_questions = mysqli_fetch_assoc($get_questions)){
	...
	$questions[$row_get_questions['survey_step_id']][$row_get_questions['id']]['question'] = ($lang_iso == 'en')? $row_get_questions['question']:$row_get_questions['question_'.$lang_iso];
    ...
    <div class="container">
		...
		<div class="langselect">
			...
			?>
					...
					<option value="<?php echo  $row_get_language['id']?>" <?php echo ($langid == $row_get_language['id'])?'selected':'';?>><?php echo $row_get_language['name']?></option>
		...
		<h2 align="center">
			...
			<?php echo ($lang_iso == 'en')?$row_get_survey['name']:$row_get_survey['name_'.$lang_iso];?>
                            ...
                            <div class="tab-pane <?php if($value['number'] == 1){ ?>active<?php } ?>" role="tabpanel" id="step<?php echo $value['number']; ?>">
										...
										<div class="col-md-6">
											...
											<div class="form-group">
												...
												<label for="locationid">
													...
													<?php echo ($lang_iso == 'en')?'Please tell us the name of the school or <br> other educational establishment travelled to or from?':'Proszę podać nam nazwę szkoły lub innej placówki oświatowej do której transport był użyty?'?>
									...
									if(!empty($row_get_survey['groups'])){ 
										...
										if($totalRows_get_group == 1){
											while($row_get_group = mysqli_fetch_assoc($get_group)){
												echo '<input type="hidden" name="locationid" value="'.$row_get_group['id'].'">';
											}
										}else{ 
											...
											<div class="col-md-6">
												<div class="form-group">
													<label for="groupid">
														<?=($lang_iso == 'en')?'group':'Lieu'?>
													</label>
													<select name="groupid" id="groupid" class="form-control form-control-lg" required>
														<option value=""><?=($lang_iso == 'en')?'Please select':'Proszę wybrać'?></option>
														<?php	
															while($row_get_group = mysqli_fetch_assoc($get_group)){	?>
														<option value="<?php echo $row_get_group['id'];?>"><?php echo ($lang_iso == 'en')?$row_get_group['name']:$row_get_group['name_'.$lang_iso];?></option>
														<?php  } ?>
													</select>
												</div>
											</div>	
											...
											<div class="col-md-6">
												<div class="form-group">
													...
													<select name="locationid" id="locationid" class="form-control form-control-lg" required>
														...
														?>
														<option value="<?php echo $row_get_location['id'];?>"><?php echo ($lang_iso == 'en')?$row_get_location['name']:$row_get_location['name_'.$lang_iso];?></option>
														<?php  } ?>
													</select>
												</div>
											</div>	
												...
												<div class="col-md-6">
													...
													<div class="form-group">
														...
														<label for="departmentid">
															...
															<?=($lang_iso == 'en')?'Department':'département'?>
														...
														<select name="departmentid" id="departmentid" class="form-control form-control-lg" required>
															...
															?>
																...
																<option value="<?php echo $row_get_department['id'];?>"><?php echo ($lang_iso == 'en')?$row_get_department['name']:$row_get_department['name_'.$lang_iso];?></option>
	                        	...
	                        	<?php 
									...
									<table class="table table-hover table-bordered">
										...
										<tbody>
											...
											while($row_get_questions_detail = mysqli_fetch_assoc($get_questions_detail)){ $tdloop++; ?>
												...
												$child_answer[$row_get_questions_detail['id']]= ($lang_iso == 'en')?$row_get_questions_detail['description']:$row_get_questions_detail['description_'.$lang_iso];

												echo ($lang_iso == 'en')?$row_get_questions_detail['description']:$row_get_questions_detail['description_'.$lang_iso];?>	
										...
										<tr>
											...
											<td colspan="<?php echo count($child_answer); ?>"><strong><?php echo ($lang_iso == 'en')?$row_get_child_questions['question']:$row_get_child_questions['question_'.$lang_iso];?></strong></td>
										...
										if($totalRows_get_answer_detail>0){
											...
											while($row_get_answer_detail = mysqli_fetch_assoc($get_answer_detail)){
												...
												$sub_answer[$row_get_answer_detail['id']]= ($lang_iso=='en')?$row_get_answer_detail['description']:$row_get_answer_detail['description_'.$lang_iso];
									...
									<?php 
										...
										$langRadioAnsVal= ($lang_iso == 'en')?$row_get_questions_detail['description']:$row_get_questions_detail['description_'.$lang_iso];		
									...
									<label class="form-check-label">
										...
										<?php echo ($lang_iso == 'en')?$row_get_questions_detail['description']:$row_get_questions_detail['description_'.$lang_iso];?> 
	                        	...
	                        	?>
									...
									<div class="form-group">
										...
										<select name="answerid[<?php echo $question['id']; ?>]" <?php if($question['ifrequired'] == 1){ ?> required <?php } ?> class="form-control subque_select" data-questionid="<?php echo $question['id']; ?>">
											...
											<option value=""><?=($lang_iso == 'en')?'Select':'Wybierz'?></option>
											...
											<option value="<?php echo $row_get_questions_detail['id'].'--'.$row_get_questions_detail['answer']; ?>"><?php echo ($lang_iso == 'en')?$row_get_questions_detail['description']:$row_get_questions_detail['description_'.$lang_iso]; ?></option>
	                    		...
	                    		<h4><?=($lang_iso == 'en')?'Can HATS Group contact you about your comments/feedback?':'Czy Grupa Hats może skontaktować się z Panem (Panią) w sprawie przesłanych nam uwag lub sugestii?'?></h4>

	                    		<div class="form-group">

	                    		    <div class="form-check contact">

									<input class="form-check-input"  type="radio" name="to_be_contact" id="to_be_contact_yes" value="1" style="visibility:hidden;"  required><?=($lang_iso == 'en')?'Yes':$langText['yes']?>
									...
									<input class="form-check-input" type="radio" name="to_be_contact" id="to_be_contact_no" value="0" style="visibility:hidden;"  required><?=($lang_iso == 'en')?'No':$langText['no']?>
								...
								<div class="form-group" id="to_be_contact_mail_div">
									...
									<div class="row">
										...
										<div class="col-md-6">
											...
											<input type="text" class="form-control fname" id="fname" name="first_name" placeholder="<?=($lang_iso == 'en')?'Your first name':$langText['your_first_name']?>" required>

										</div>

										<div class="col-md-6">

											<input type="text" class="form-control" id="lname" name="last_name" placeholder="<?=($lang_iso == 'en')?'Your last name':$langText['your_last_name']?>" required>
									...
									<div class="row">
										...
										<div class="col-md-6">
											...
											<input type="email" class="form-control" id="to_be_contact_mail" placeholder="<?=($lang_iso == 'en')?'Your email':$langText['your_email']?>" name="to_be_contact_mail" required> 

										</div>

										<div class="col-md-6">

											<input type="number" class="form-control" id="phone" name="phone_number" placeholder="<?=($lang_iso == 'en')?'Your phone number':$langText['your_phone_number']?>" required>
                                ...
                                <ul class="list-inline text-center">
		                            ...
		                            <li>
										...
										<button type="button" class="default-btn next-step">
											...
											<?=($lang_iso == 'en')?'Continue to next step':$langText['continue_next_step']?>
		                            ...
		                            <li>
										...
										<?=($lang_iso == 'en')?'Back':$langText['back']?>
		                            ...
		                            <li><input type="submit" name="submit" class="default-btn finalSubmit submitform" id="submit" value="<?=($lang_iso == 'en')?'Finish':$langText['finish']?>"></li>
		                            ...
		                            <li>
										...
										<button type="button" class="default-btn next-step">
											...
											<?=($lang_iso == 'en')?'Continue':$langText['continue']?>
...
$(document).ready(function () {
	...
	$.extend($.validator.messages, {
	    ...
	    required: "<?= ($lang_iso == 'en') ? 'This field is required' : $langText['required_error']; ?>"
